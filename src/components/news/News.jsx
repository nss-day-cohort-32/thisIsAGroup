...
export default class News extends Component {
  ...
  confirmDelete = newsId => {
    API.deleteNews(newsId).then(_reply => {
      API.getUserNews(sessionStorage.getItem("activeUser")).then(news => {
        this.setState({ news });
      });
    });
  };
  ...
  editNews = (id, obj) => {
    API.editNews(id, obj).then(_reply => {
      API.getUserNews(sessionStorage.getItem("activeUser")).then(news => {
        this.setState({ news });
      });
    });
  ...
  async componentDidMount() {
    const newState = {
      news: await API.getUserNews(sessionStorage.getItem("activeUser")).catch(
        _error => []
      )
    };
    ...
    return (
      <>
        <Grid
          ...
          <Grid item>
            <Typography variant="h3">Breaking News:</Typography>
          </Grid>
          <Grid item>
